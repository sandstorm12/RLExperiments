...
class DQN:
    ...
    def learn(self, num_iterations, batch_size=32,
            novel_interactions=8):
    ...
    def _report(self, iteration, samples, bar, period=100):
    ...
    def _train_networks(self, samples):
        ...
        states_next = torch.tensor(
            np.array([sample[3] for sample in samples]))\
            .float().permute(0, 1, 2, 3, 4)

        q_current = self._network_main(states.to(self._device)).cpu().gather(
            1, actions.view(-1, 1)).view(-1)
        
        q_target = self._network_target(states_next.to(self._device)).cpu()
        q_best = (rewards + self._discount_factor *
            torch.max(q_target, dim=1)[0]).float()

        self._network_main.train_step(q_current.float(), q_best.float())
